...
// Website 設計文件 §2.5/§3.3:主按鈕(accent 底 + on-accent 文字)、次按鈕
// (surface 底 + border-control 邊線 + accent 文字);兩者共用點擊區
// ≥44×44px、focus-visible 外框、disabled 同時改色又改語意狀態的規則。
...
const buttonClass = computed(() => [
  'inline-flex min-h-[44px] min-w-[44px] items-center justify-center gap-2 rounded-lg px-5 py-3 text-btn font-medium transition-colors duration-200',
  ...
  props.variant === 'primary'
    ...
    : 'border border-wy-border-control bg-wy-surface text-wy-accent hover:bg-wy-surface-subtle disabled:border-wy-disabled-bg disabled:text-wy-disabled-text',
])
</script>
